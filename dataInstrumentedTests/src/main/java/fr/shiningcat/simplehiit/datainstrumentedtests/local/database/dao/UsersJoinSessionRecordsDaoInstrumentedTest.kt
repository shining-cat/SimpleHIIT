...
@HiltAndroidTest
class UsersJoinSessionRecordsDaoInstrumentedTest {
    @get:Rule
...
// //////////////
    ...
    @Test
    fun cascadeSessionsDeletionWhenDeletingAUser() =
        runTest {
            ...
            val sessionsForUser1AfterDeletion =
                sessionRecordsDao.getSessionsForUser(userId = testUserID)
            assertEquals(0, sessionsForUser1AfterDeletion.size)
            // assert sessions for user 2 are still all there
            val sessionsForUser2AfterDeletion =
                sessionRecordsDao.getSessionsForUser(userId = testUserID2)
    ...
    @Test
    fun cascadeSessionsDeletionWhenDeletingAllUsers() =
        runTest {
            // first insert users in users table
            val testUser1 =
                UserEntity(userId = testUserID, name = testUserName, selected = testSelected)

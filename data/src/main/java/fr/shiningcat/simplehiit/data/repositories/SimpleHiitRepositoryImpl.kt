...
class SimpleHiitRepositoryImpl @Inject constructor(
    ...
    override fun getUsers(): Flow<Output<List<User>>> {
        return try {
            ...
            flowOf(
                Output.Error(errorCode = Constants.Errors.DATABASE_FETCH_FAILED, exception = exception)
            )
        }
    }
    ...
    override suspend fun deleteUser(user: User): Output<Int> {
        return withContext(ioDispatcher) {
            try {
                ...
                Output.Error(errorCode = Constants.Errors.DATABASE_DELETE_FAILED, exception = exception)
            }
        }
    }

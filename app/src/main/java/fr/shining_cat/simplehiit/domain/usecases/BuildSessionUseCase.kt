...
class BuildSessionUseCase @Inject constructor(
    ...
    suspend fun execute(
        ...
        val totalSessionLengthMs = sessionSettings.cycleLengthMs.times(sessionSettings.numberCumulatedCycles)
    ...
    private suspend fun getSelectedExercisesTypesList(sessionSettings: SessionSettings): List<Exercise>{
        ...
        return composeExercisesListForSessionUseCase.execute(
    ...
    private suspend fun buildStepsList(
        ...
        if(sessionStartCountDownLengthMs > 0L){
            val totalSessionDurationMs = totalSteps.times(workPeriodLengthMs.plus(restPeriodLengthMs))
        ...
        for ((index, exercise) in exercisesList.withIndex()) {
            ...
            val restStepDurationFormatted = formatLongDurationMsAsSmallestHhMmSsStringUseCase.execute(
                restPeriodLengthMs,
                durationStringFormatter
            )
            val workStepDurationFormatter = formatLongDurationMsAsSmallestHhMmSsStringUseCase.execute(
                workPeriodLengthMs,
                durationStringFormatter
            )
            val remainingSessionDurationMsAfterRest = (remainingExercisesAfterStep.plus(1)).times(workPeriodLengthMs) + (remainingExercisesAfterStep).times(restPeriodLengthMs)
            hiitLogger.d("BuildSessionUseCase", "buildStepsList::remainingSessionDurationMsAfterRest = $remainingSessionDurationMsAfterRest")
            val remainingSessionDurationMsAfterWork = remainingExercisesAfterStep.times(workPeriodLengthMs.plus(restPeriodLengthMs))

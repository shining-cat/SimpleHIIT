...
internal class StepTimerUseCaseTest : AbstractMockkTest() {
    @Test
    fun `timer runs for expected duration and emits expected states in order`() =
        runTest {
            ...
            for (i in 0..expectedTicksCount - 2) {
                expectedStatesEmitted.add(
                    StepTimerState(
                        totalMilliSeconds - tickLength.times(i),
                        totalMilliSeconds,
                    ),
                )
            }
 ...
 */
internal class TestTimeProvider(
    startClock: Long = 0L,

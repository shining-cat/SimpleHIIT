object Versions {
    const val appCompat = "1.7.0"
    const val datastore = "1.1.1"
    const val material = "1.12.0"
    const val jupiter = "5.11.2"
    const val junit = "1.2.1"
    const val androidXcoreKtx = "1.10.1"
    const val hilt = "2.52"
    const val hiltNavigation = "1.2.0"
    const val room_version = "2.6.1"
    const val composeMaterial3WindowSizeClass = "1.1.2"
    const val compose_nav_version = "2.8.2"
    const val mockkVersion = "1.13.12"
    const val coroutinesTest = "1.9.0"
    const val core_testing = "2.2.0"
    const val test_runner = "1.6.2"
    const val coil = "2.7.0"
    const val benManesDependenciesVersionPlugin = "0.51.0"
    const val gmazzoJacocoReportAggregationPlugin = "2.2.1"
    const val savvasdalkitsisDependencyGraphPlugin = "0.12"
    const val composeBom = "2024.09.03"

    // see https://developer.android.com/jetpack/androidx/releases/tv for releases announcements:
    const val materialTV = "1.0.0"
    const val composeTV = "1.0.0-alpha11"

    // this has to be kept in sync with kotlin-gradle-plugin version. See https://developer.android.com/jetpack/androidx/releases/compose-kotlin for kotlin and compose compile version compatibility:
    const val kotlinCompilerExtension = "1.5.15"
    const val ktlintGradlePlugin = "12.1.1"

    // see https://developer.android.com/jetpack/androidx/releases/lifecycle for releases announcements:
    const val androidxLifeCycleProcess = "2.8.6"
}

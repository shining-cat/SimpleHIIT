...
internal class CreateUserUseCaseTest : AbstractMockkTest() {
    ...
    fun `if name is not used by any other user calls repo with corresponding value and returns repo success`() =
        runTest {
            ...
            val successFromRepo = fr.shining_cat.simplehiit.commondomain.Output.Success(1L)
            coEvery { mockSimpleHiitRepository.insertUser(any()) } answers { successFromRepo }
            val successFromNameCheck = fr.shining_cat.simplehiit.commondomain.Output.Success(false)
    ...
    fun `return corresponding error if checking name fails with error`() = runTest {
        ...
        val errorFromRepo = fr.shining_cat.simplehiit.commondomain.Output.Error(fr.shining_cat.simplehiit.commondomain.Constants.Errors.EMPTY_RESULT, Exception(exceptionMessage))
    ...
    fun `return correct error if name is already taken`() = runTest {
        ...
        val testValue = User(id = 123L, name = "test user name", selected = true)
        val successFromRepo = fr.shining_cat.simplehiit.commondomain.Output.Success(true)
        ...
        coVerify(exactly = 1) { mockCheckIfAnotherUserUsesThatNameUseCase.execute(testValue) }
        coVerify(exactly = 0) { mockSimpleHiitRepository.insertUser(testValue) }
        val expectedErrorCode = fr.shining_cat.simplehiit.commondomain.Constants.Errors.USER_NAME_TAKEN
        val expectedExceptionCode = fr.shining_cat.simplehiit.commondomain.Constants.Errors.USER_NAME_TAKEN.code
        assertTrue(result is fr.shining_cat.simplehiit.commondomain.Output.Error)
        result as fr.shining_cat.simplehiit.commondomain.Output.Error
    ...
    fun `if name is not used by any other user but insertion fails returns repo insertion error`() =
        runTest {
            ...
            val exceptionMessage = "this is a test exception"
            val errorFromRepo =
                fr.shining_cat.simplehiit.commondomain.Output.Error(fr.shining_cat.simplehiit.commondomain.Constants.Errors.EMPTY_RESULT, Exception(exceptionMessage))

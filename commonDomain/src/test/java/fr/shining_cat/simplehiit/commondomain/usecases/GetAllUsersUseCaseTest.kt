...
internal class GetAllUsersUseCaseTest : AbstractMockkTest() {
    ...
    fun `calls repo and return corresponding values in order`() = runTest {
        ...
        //
        val usersFlowAsList = mutableListOf<fr.shining_cat.simplehiit.commondomain.Output<List<User>>>()
    ...
    fun `calls repo and return error if repo returns empty list`() = runTest {
        val usersEmptyList = fr.shining_cat.simplehiit.commondomain.Output.Success(emptyList<User>())
        val usersFlow = MutableSharedFlow<fr.shining_cat.simplehiit.commondomain.Output<List<User>>>()
        coEvery { mockSimpleHiitRepository.getUsers() } answers { usersFlow }
        //
        val usersFlowAsList = mutableListOf<fr.shining_cat.simplehiit.commondomain.Output<List<User>>>()
        ...
        assertTrue(users is fr.shining_cat.simplehiit.commondomain.Output.Error)
        users as fr.shining_cat.simplehiit.commondomain.Output.Error
        assertEquals(fr.shining_cat.simplehiit.commondomain.Constants.Errors.NO_USERS_FOUND, users.errorCode)
        assertEquals(fr.shining_cat.simplehiit.commondomain.Constants.NO_RESULTS_FOUND, users.exception.message)
        //
        collectJob.cancel()
    }

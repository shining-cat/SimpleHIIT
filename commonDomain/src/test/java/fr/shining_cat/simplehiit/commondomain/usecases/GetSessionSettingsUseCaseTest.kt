...
internal class GetSessionSettingsUseCaseTest : AbstractMockkTest() {
    ...
    private val testedUseCase = GetSessionSettingsUseCase(mockSimpleHiitRepository, mockHiitLogger)
    ...
    fun `calls repo and return success with correct order of values`() = runTest(UnconfinedTestDispatcher()) {
        ...
        val usersList1 = fr.shining_cat.simplehiit.commondomain.Output.Success(listOf(user1, user3))
        val usersList2 = fr.shining_cat.simplehiit.commondomain.Output.Success(listOf(user1, user2, user4))
        val usersFlow = MutableSharedFlow<fr.shining_cat.simplehiit.commondomain.Output<List<User>>>()
        coEvery { mockSimpleHiitRepository.getSelectedUsers() } answers { usersFlow }
        //
        val generalSettingsFlowAsList = mutableListOf<fr.shining_cat.simplehiit.commondomain.Output<SessionSettings>>()
        ...
        val expectedResult1 = fr.shining_cat.simplehiit.commondomain.Output.Success(
            SessionSettings(
        ...
        val expectedResult2 = fr.shining_cat.simplehiit.commondomain.Output.Success(
            SessionSettings(
        ...
        val expectedResult3 = fr.shining_cat.simplehiit.commondomain.Output.Success(
            SessionSettings(
    ...
    fun `calls repo and return error if repo returns error for Users`() = runTest(UnconfinedTestDispatcher()) {
        ...
        val usersError = fr.shining_cat.simplehiit.commondomain.Output.Error(fr.shining_cat.simplehiit.commondomain.Constants.Errors.DATABASE_FETCH_FAILED, testException)
        val usersFlow = MutableSharedFlow<fr.shining_cat.simplehiit.commondomain.Output<List<User>>>()
        coEvery { mockSimpleHiitRepository.getSelectedUsers() } answers { usersFlow }
        //
        val generalSettingsFlowAsList = mutableListOf<fr.shining_cat.simplehiit.commondomain.Output<SessionSettings>>()
        ...
        val expectedError = fr.shining_cat.simplehiit.commondomain.Output.Error(fr.shining_cat.simplehiit.commondomain.Constants.Errors.NO_USERS_FOUND, testException)
